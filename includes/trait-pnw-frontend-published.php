...
trait PNW_Frontend_Published_Trait {
    ...
    private static function render_published_news(): void {
        ...
        echo '<div class="pnw-section-heading"><div><div class="pnw-kicker">Publikált tartalom</div><h3>Kint lévő hírek</h3><p>A lista a régi Petrik-bejegyzéseket és a Hírkezelőből publikált híreket is tartalmazza. Az Archiválás leveszi a hírt a weboldalról, de nem törli.</p></div><a class="pnw-button pnw-button-secondary" href="' . esc_url( PNW_Plugin::manager_url( array( 'pnw_view' => 'archive' ) ) ) . '">Archívum</a></div>';

        if ( $test_mode ) {
            echo '<div class="pnw-notice pnw-notice-warning"><strong>TESZT MÓD:</strong> a jelenlegi éles hírek megtekinthetők, de szerkesztésük és archiválásuk blokkolva van a production indulásig.</div>';
        }
        ...
        foreach ( $query->posts as $post ) {
            ...
            if ( ! $test_mode ) {
                echo '<form class="pnw-inline-action-form" method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" data-confirm="Biztosan archiválod ezt a hírt? Az oldalról azonnal eltűnik, de az Archívumból bármikor visszaállítható.">';
                echo '<input type="hidden" name="action" value="pnw_archive_published_news"><input type="hidden" name="post_id" value="' . esc_attr( (string) $post->ID ) . '">';
                wp_nonce_field( 'pnw_archive_published_news', 'pnw_nonce' );
                echo '<button class="pnw-button pnw-button-danger" type="submit">Archiválás</button></form>';
            }
            echo '</div>';
            echo '</td></tr>';
        }
    ...
    private static function render_archived_news(): void {
        if ( ! current_user_can( 'pnw_manage_published_news' ) ) {
            self::render_unauthorized();
            return;
        }
        ...
        echo '<section class="pnw-section">';
        echo '<div class="pnw-section-heading"><div><div class="pnw-kicker">Nem publikus tartalom</div><h3>Archívum</h3><p>Az itt lévő hírek nem jelennek meg a weboldalon, de teljes tartalmuk megmarad és egy kattintással visszaállíthatók.</p></div><a class="pnw-button pnw-button-secondary" href="' . esc_url( PNW_Plugin::manager_url( array( 'pnw_view' => 'published' ) ) ) . '">← Kint lévő hírek</a></div>';

        echo '<form class="pnw-form pnw-published-search-card" method="get" action="' . esc_url( PNW_Plugin::manager_url() ) . '">';
        echo '<input type="hidden" name="pnw_view" value="archive">';
        echo '<div class="pnw-field" style="margin-bottom:0"><label for="pnw-archive-search">Keresés az archívumban</label><div class="pnw-published-search-row"><input id="pnw-archive-search" type="search" name="pnw_q" value="' . esc_attr( $search ) . '" placeholder="Hír címe vagy szövege"><button class="pnw-button pnw-button-secondary" type="submit">Keresés</button></div></div>';
        echo '</form>';

        if ( ! $query->have_posts() ) {
            echo '<div class="pnw-empty-inline">Az archívum jelenleg üres.</div>';
            echo '</section>';
            wp_reset_postdata();
            return;
        }

        echo '<div class="pnw-table-card pnw-published-card"><div class="pnw-table-wrap"><table class="pnw-table pnw-published-table"><thead><tr><th>Hír</th><th>Szerző</th><th>Eredeti publikálás</th><th>Archiválva</th></tr></thead><tbody>';
        foreach ( $query->posts as $post ) {
            $author      = get_userdata( (int) $post->post_author );
            $archived_at = (string) get_post_meta( $post->ID, '_pnw_archived_at', true );
            $archived_by = absint( get_post_meta( $post->ID, '_pnw_archived_by', true ) );
            $archiver    = $archived_by ? get_userdata( $archived_by ) : null;

            echo '<tr class="pnw-published-main-row">';
            echo '<td data-label="Hír"><strong class="pnw-published-title">' . esc_html( $post->post_title ?: '(Névtelen hír)' ) . '</strong><small class="pnw-published-meta">' . esc_html( self::category_names( (int) $post->ID ) ) . '</small></td>';
            echo '<td data-label="Szerző">' . esc_html( $author ? $author->display_name : '—' ) . '</td>';
            echo '<td data-label="Eredeti publikálás">' . esc_html( get_the_date( 'Y.m.d. H:i', $post ) ) . '</td>';
            echo '<td data-label="Archiválva">' . esc_html( $archived_at ?: get_the_modified_date( 'Y.m.d. H:i', $post ) ) . ( $archiver ? '<small class="pnw-published-meta">' . esc_html( $archiver->display_name ) . '</small>' : '' ) . '</td>';
            echo '</tr>';

            echo '<tr class="pnw-published-action-row"><td colspan="4"><div class="pnw-published-inline-actions">';
            echo '<form class="pnw-inline-action-form" method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" data-confirm="Biztosan visszaállítod ezt a hírt? A hír azonnal újra megjelenik a nyilvános weboldalon.">';
            echo '<input type="hidden" name="action" value="pnw_restore_archived_news"><input type="hidden" name="post_id" value="' . esc_attr( (string) $post->ID ) . '">';
            wp_nonce_field( 'pnw_restore_archived_news', 'pnw_nonce' );
            echo '<button class="pnw-button" type="submit">Visszaállítás és újrapublikálás</button></form>';
            echo '</div></td></tr>';
        }
        echo '</tbody></table></div></div>';
        ...
        if ( $query->max_num_pages > 1 ) {
            echo '<div class="pnw-published-pagination">';
            if ( $paged > 1 ) {
                $prev_args = array( 'pnw_view' => 'archive', 'pnw_page' => $paged - 1 );
                if ( '' !== $search ) {
                    $prev_args['pnw_q'] = $search;
                }
                echo '<a class="pnw-button pnw-button-secondary" href="' . esc_url( PNW_Plugin::manager_url( $prev_args ) ) . '">← Előző</a>';
            ...
            echo '<span>' . esc_html( (string) $paged ) . ' / ' . esc_html( (string) $query->max_num_pages ) . ' oldal</span>';
            if ( $paged < (int) $query->max_num_pages ) {
                $next_args = array( 'pnw_view' => 'archive', 'pnw_page' => $paged + 1 );
                if ( '' !== $search ) {
                    $next_args['pnw_q'] = $search;
                }
                echo '<a class="pnw-button pnw-button-secondary" href="' . esc_url( PNW_Plugin::manager_url( $next_args ) ) . '">Következő →</a>';
    ...
    private static function render_published_editor( int $post_id ): void {
        ...
        if ( $test_mode ) {
            echo '<div class="pnw-notice pnw-notice-warning"><strong>TESZT MÓD:</strong> ez valódi, jelenleg publikus tartalom. A production indulásig ezen a felületen csak megtekinthető, nem módosítható és nem archiválható.</div>';
        ...
        echo '<form class="pnw-delete-form" method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" data-confirm="Biztosan archiválod ezt a jelenleg publikus hírt? Az oldalról azonnal eltűnik, de az Archívumból bármikor visszaállítható.">';
        echo '<input type="hidden" name="action" value="pnw_archive_published_news"><input type="hidden" name="post_id" value="' . esc_attr( (string) $post_id ) . '">';
        wp_nonce_field( 'pnw_archive_published_news', 'pnw_nonce' );
        echo '<button class="pnw-button pnw-button-danger" type="submit">Archiválás</button></form>';
        echo '</section>';
    }
